...
public class Main {
	...
	private static final int[] source = ArrayUtils.randomArray(n);
	private static int[] expected;
	
	public static void main(String[] args)
	{
		expected = ArrayUtils.clone(source);
...
//		runSort(new InPlaceCountingSort(range));
	...
	private static void runSort(Sorter sorter) {
		int[] array = null;
		try {
			array = ArrayUtils.clone(source);
			long t = System.nanoTime();
			sorter.sort(array);
			System.out.printf("%,12d %s\n", System.nanoTime()-t, sorter);
			ArrayUtils.AssertEquals(expected, array);
		} catch(Throwable t) {
